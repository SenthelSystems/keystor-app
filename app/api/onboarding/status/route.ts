...
export async function GET() {
  try {
    ...
    const [propertiesCount, unitsCount, tenantsCount, activeLeaseCount] = await Promise.all([
    ...
    return NextResponse.json({ error: e?.message ?? "Failed to load onboarding status" }, { status });
  }
}
